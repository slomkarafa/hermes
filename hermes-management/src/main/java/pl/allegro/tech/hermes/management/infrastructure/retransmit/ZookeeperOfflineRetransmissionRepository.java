        ...
        try {
            if (pathExists(paths.offlineRetransmissionPath())) {
                return zookeeper.getChildren()
                        ...
                        .stream()
                        .map(id -> {
                            OfflineRetransmissionRequest request = readFrom(paths.offlineRetransmissionPath(id), OfflineRetransmissionRequest.class);
                            return new OfflineRetransmissionTask(id, request);
